...
function M.textedit_kb()
   ...
   kb('n', '  ', '<Cmd>nohlsearch<Bar>diffupdate<CR>',
      { desc = 'clear hlsearch' })
...
function M.telescope_kb()
   ...
   kb('n', ' tz', tb.current_buffer_fuzzy_find,
...
function M.lsp_kb(client, bufnr)
   ...
   kb('n', 'zK', vim.lsp.buf.signature_help,
      { desc = 'signature help', buffer = bufnr })
   kb('n', 'K', vim.lsp.buf.hover,
      { desc = 'hover', buffer = bufnr })
   kb('n', 'zqd', vim.diagnostic.setqflist,
      { desc = 'qf list ws diagnostics', buffer = bufnr })
   kb('n', 'zqe',
      function()
         vim.diagnostic.setqflist { severity = 'E' }
      end, { desc = 'qf list ws errors', buffer = bufnr })
   kb('n', 'zqw',
      function()
         vim.diagnostic.setqflist { severity = 'W' }
      end, { desc = 'qf list ws warnings', buffer = bufnr })
   kb('n', 'zr', vim.lsp.buf.rename, { desc = 'rename', buffer = bufnr })
   kb('n', 'zfa', vim.lsp.buf.add_workspace_folder,
      { desc = 'add workspace folder', buffer = bufnr })
   kb('n', 'zfr', vim.lsp.buf.remove_workspace_folder,
      { desc = 'remove workspace folder', buffer = bufnr })
   kb('n', '[',
      function()
         vim.diagnostic.goto_prev { wrap = false }
      end, { desc = 'diagnostic goto previous', buffer = bufnr })
   kb('n', ']',
      function()
         vim.diagnostic.goto_next { wrap = false }
      end, { desc = 'diagnostic goto next', buffer = bufnr })
